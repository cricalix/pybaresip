...
@tdsl.context
def baresip_vidloop(context: DSLContext) -> None:
    context.shared_context(PyBareSIPContext)
    context.merge_context("PyBareSIPContext")

    @context.sub_context
    def when_vidloop_is_called(context: DSLContext) -> None:
        @context.before
        ...
        @context.example
        async def it_calls_invoke_with_vidloop(self: ContextData) -> None:
            await self.bs.vidloop()
