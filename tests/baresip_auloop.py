...
@tdsl.context
def baresip_auloop(context: DSLContext) -> None:
    context.shared_context(PyBareSIPContext)
    context.merge_context("PyBareSIPContext")

    @context.sub_context
    def when_auloop_is_called(context: DSLContext) -> None:
        @context.before
        async def before(self: ContextData) -> None:
            self.mock_async_callable(target=self.bs, method="invoke").to_return_value(
                "None"
            ).for_call("auloop").and_assert_called_once()

        @context.example
        async def it_calls_invoke_with_auloop(self: ContextData) -> None:
            await self.bs.auloop()
